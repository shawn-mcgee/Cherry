...
public class Entity implements Renderable, Updateable {
	...
	public void onUpdate(UpdateContext context) {
		...
		if(segment_a != null) {
		    float 
		    	t = Vector.s_pro(Vector.sub(local, segment_a.v1), segment_a.dv);
		    Vector2 
		    	p = Vector.add(segment_a.v1, Vector.mul(segment_a.dv, Util.clamp(t, 0f, 1f))); 
		    float 
		    	d = Vector.dot(Vector.sub(local, p));
		    if(d < delta) {
		    	point = p;
		    	delta = d;
		    }
		...
		if(segment_b != null) {
		    float 
		    	t = Vector.s_pro(Vector.sub(local, segment_b.v1), segment_b.dv);
		    Vector2
		...
		if(point != null && delta < size * size) {
			...
			//solve for dx
			{
				float a0 = dx * dx;
				if(a0 != 0) {
					float b0 = 2 * dx * (local.x() - point.x());
					float
						b1 = b0 / a0,
						c1 = c0 / a0,
						q0 = b1 / 2 ,
						q1 = (float)Math.sqrt(q0 * q0 - c1);
					tx = Util.clamp(Math.min(-q0 + q1, -q0 - q1), 0f, 1f);
				}
			}
			...
			//solve for dy
			{
				float a0 = dy * dy;
				if(a0 != 0) {
					float b0 = 2 * dy * (local.y() - point.y());
					float
						b1 = b0 / a0,
						c1 = c0 / a0,
						q0 = b1 / 2 ,
						q1 = (float)Math.sqrt(q0 * q0 - c1);
					ty = Util.clamp(Math.min(-q0 + q1, -q0 - q1), 0f, 1f);
				}
			}
